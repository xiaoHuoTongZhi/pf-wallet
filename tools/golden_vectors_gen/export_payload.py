...
def encode(manifest: dict, stages: dict[str, list[dict]], generated_at: int) -> dict:
    ...
    for stage in STAGE_ORDER:  # 行序由 STAGE_ORDER 决定，与传入键序无关
        for fields in stages.get(stage, []):
            record_bytes += dumps_line({"type": stage, **fields})
            record_count += 1
    ...
    assert lines[0]["type"] == "manifest" and lines[-1]["type"] == "end"
...
def build_suite() -> dict:
    ...
    stages_full = {
        ...
        "account": [{"id": "01J8TESTACCNT00000000000001", "ledgerId": "01J8TESTLEDGER0000000000001", "name": "招行储蓄卡", "type": 2, "currency": "CNY", "openingBalanceMinor": 0, "cachedBalanceMinor": 1284500, "balanceAsOf": 1757900000000, "creditLimitMinor": None, "statementDay": None, "dueDay": None, "repayAccountId": None, "icon": "card", "color": "#3B82F6", "isArchived": 0, "sortOrder": 1, "note": None, "createdAt": 1, "updatedAt": 1, "deletedAt": None, "deviceId": "01J8Z9K2M4P6Q8R0T2V4X6Z8B1", "rev": 1}],
