...
| schema v1 DDL + 迁移执行器（§2.3 / §2.7，`pf_data/schema_v1.dart`、`migration_runner.dart`） | ✅ 已落地（纯 Dart） | DDL 结构断言（逐表逐索引逐触发器）+ 执行器编排契约单测（一步一事务 / checksum 留痕 / user_version 原子推进 / 幂等） |
| 仓储 + 余额推演（§2.1 / §2.3，`pf_data/*_repository.dart`、`balance_engine.dart`） | ✅ 已落地（纯 Dart） | 余额推演被 `balance_replay` 向量锁死（期望值由独立 Python 实现生成）：转账双腿 / 信用卡负债语义 / 软删回退 / 增量与全量重算交叉校验 / 随机 1000 笔不变量；转账不进收支合计同样入向量 |

§7.6 第 ① 项 `pf_crypto` 原语层**完成**；第 ② 项数据层的打开流程编排、
schema v1 与迁移执行器、三仓储与余额推演也已落地。
